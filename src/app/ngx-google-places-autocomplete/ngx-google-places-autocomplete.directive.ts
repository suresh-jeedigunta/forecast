...
export class GooglePlaceDirective implements AfterViewInit {
    ...
    @Input('options') set options(opt:{type:string})
    {
        console.log("GooglePlaceDirective set",opt);
        this.options_  =new Options({types: [opt.type]});
    ...
    ngAfterViewInit(): void {
        if (!this.options_)
            this.options_  =new Options({types: ['geocode']});
    ...
    private initialize(): void {
        ...
        this.autocomplete = new google.maps.places.Autocomplete(this.el.nativeElement, this.options_);

        if (!this.autocomplete)
        {
            throw new Error("Autocomplete is not initialized");
        }
        this.autocomplete.setFields(['address_component','geometry.location','place_id']);
    ...
    public reset(): void {
        this.autocomplete.setComponentRestrictions(this.options_.componentRestrictions);
        this.autocomplete.setTypes(this.options_.types);
    }
